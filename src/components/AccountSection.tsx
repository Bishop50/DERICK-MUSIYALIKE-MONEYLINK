...
const AccountSection: React.FC<AccountSectionProps> = ({ onBack, onLogout, currentUser, onUpdateUser, onNavigate, config }) => {
  ...
  const menuItems = [
    ...
    { id: 'referral', label: 'Referral Program', icon: Users, onClick: () => toast.info('Referral Code: DMI' + currentUser?.id?.toUpperCase()) },
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white p-8 rounded-[2rem] border border-[#E5E5E5] space-y-6">
          ...
          <div className="space-y-4">
            <button 
              onClick={() => toast.success('Password reset link sent to your registered phone number.')}
